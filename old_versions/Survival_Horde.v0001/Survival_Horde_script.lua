...
--{395, (ShuffleList(AllTransportSpawns))[1], 12, 'None', 
--"IssueAttackFormation", ValidGunshipTargetsB, nil},
...
Survival_InitGame = function()
    ...
    local EnemyCommander = CreateUnitHPR('URL0001', "ARMY_SURVIVAL_ENEMY", 10, 13.5, -10, 0, 0, 0)  --create commander for ARMY_SURVIVAL_ENEMY
	...
	EnableLane5 = 0
	EnableLane3 = 1
	--LOG("Only one player, activated lane 3")
	end
	
	--LOG("ValidGunshipTargetsA Before")
	--ValidGunshipTargetsA = ShuffleList(BaseRaidingTargets)
	--for i = 1, table.getn(ValidGunshipTargetsA) do
	--LOG(ValidGunshipTargetsA[i])
	--end
	--LOG("ValidGunshipTargetsB Before")
	--ValidGunshipTargetsB = ShuffleList(BaseRaidingTargets)
	--for i = 1, table.getn(ValidGunshipTargetsB) do
	--LOG(ValidGunshipTargetsB[i])
	--end
	--LOG("ValidGunshipTargetsA After")
	--ValidGunshipTargetsA = ShuffleList(ValidGunshipTargetsA)
	--for i = 1, table.getn(ValidGunshipTargetsA) do
	--LOG(ValidGunshipTargetsA[i])
	--end
	--LOG("ValidGunshipTargetsB After")
	--ValidGunshipTargetsB = ShuffleList(ValidGunshipTargetsB)
	--for i = 1, table.getn(ValidGunshipTargetsB) do
	--LOG(ValidGunshipTargetsB[i])
	--end
...
CheckFocusArmy = function()
	...
	if (GetFocusArmy() < 1) then
		...
		SimObjects.GetFocusArmy = function()      -- <<<<< Fix to spectating not working
...
local OncePerTick =  math.floor(GetGameTimeSeconds()) 
	...
	while (GameHasEnded == 0) do
		...
		if (OncePerTick < GameTime) then
			...
			--if (GameTime == 2) then
				--ValidGunshipTargetsA = BaseRaidingTargets
				--ValidGunshipTargetsB = BaseRaidingTargets
				--ShuffleList(ValidGunshipTargetsA)
				--ShuffleList(ValidGunshipTargetsB)
			...
			local UnitListLand = GetArmyBrain("ARMY_SURVIVAL_ENEMY"):GetListOfUnits( categories.LAND + categories.DESTROYER, true, false) 
			...
			if (table.getn(IdleUnitListLand) > 0) then
				...
				aiPlatoonLand:AggressiveMoveToLocation(ScenarioUtils.MarkerToPosition((ShuffleList(LandCenter))[2])) 
				aiPlatoonLand:AggressiveMoveToLocation(ScenarioUtils.MarkerToPosition((ShuffleList(LandCenter))[3])) 
				aiPlatoonLand:DisbandOnIdle()

			end

			------------------------------------------------------------AIR UNIT IDLE PREVENTER!---------------------------------------------------------------------------------------------------------------------------------------------------------------------
			local RandomFighterRoute = Random(1, table.getn(AllBomberBaseRoutes))
			local IdleUnitsScoutFighter = {} 
			local UnitListAir = aiBrain:GetListOfUnits( categories.AIR - categories.TRANSPORTATION - categories.ANTINAVY - categories.BOMBER - categories.GROUNDATTACK - categories.INTELLIGENCE, false, false) 
			local aiPlatoonAir = aiBrain:MakePlatoon('','') 
			aiPlatoonAir:DisbandOnIdle()

			for k, IdleUnit in UnitListAir do

				if (IdleUnit:IsIdleState()) then
					table.insert(IdleUnitsScoutFighter, IdleUnit)
				end

			end

			if (table.getn(IdleUnitsScoutFighter) > 0) then
				aiBrain:AssignUnitsToPlatoon(aiPlatoonAir, IdleUnitsScoutFighter, 'Attack', 'None') 
				for i = 1, table.getn(AllBomberBaseRoutes[RandomFighterRoute]) do
				
					aiPlatoonAir:AggressiveMoveToLocation(ScenarioUtils.MarkerToPosition(AllBomberBaseRoutes[RandomFighterRoute][i])) 

				end
				aiPlatoonAir:MergeWithNearbyPlatoons('', 80)
				aiPlatoonAir:DisbandOnIdle()
			end
			...
			------------------------------------------------------------BOMBER AND GUNSHIP AND SCOUT IDLE PREVENTER!---------------------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			local UnitListBomber = (aiBrain:GetListOfUnits(categories.AIR - categories.ANTIAIR - categories.TRANSPORTATION - categories.ANTINAVY + categories.GROUNDATTACK, false, false)) 
					...
					if (table.getn(coms) == 0) then
						...
						for k, com in coms do
							...
							local cdrDist = VDist2(cdrPOS[1], cdrPOS[3], 249.50, 288.50) 
								...
								if (cdrDist > 150 and cdrPOS[2] >= 20) then
								...
								elseif (cdrDist <= 150 and cdrPOS[2] >= 20) then
...
function ShuffleListPartially(list, startingID, endingID, IgnoreList)
...
GiveMoveOrder = function(aiPlatoon, MarkerList)
...
IssueStrategicMissile = function(UnitList, TargetList)
